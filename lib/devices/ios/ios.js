...
IOS.prototype.configure = function (args, caps, cb) {
  ...
  if (!this.args.app &&
      !((ios8 || this.onRealDevice()) && this.args.bundleId)) {
...
IOS.prototype.onRealDevice = function() {
  return this.args.udid && !this.shouldUseSpecificSimulator();
};

IOS.prototype.shouldUseSpecificSimulator = function(){
  return this.args.specificSimulator;
};
...
IOS.prototype.makeInstruments = function (cb) {
  ...
  var bootstrap = prepareBootstrap({
    ...
    sendKeyStrategy: this.args.sendKeyStrategy || (this.onRealDevice() ? 'grouped' : 'oneByOne')
  });

  bootstrap.then(function (bootstrapPath) {
      var instruments = new Instruments({
        // on real devices bundleId is always used
        app: (!this.onRealDevice() ? this.args.app : null) || this.args.bundleId
      , udid: this.args.udid
...
IOS.prototype.detectUdid = function (cb) {
  ...
  if (this.onRealDevice() && this.args.udid === "auto") {
...
IOS.prototype.checkSimAvailable = function (cb) {
  ...
  if (this.shouldUseSpecificSimulator()) {
    logger.debug("Not checking whether simulator is available since --udid " +
        "is specified");
    this.iOSSimUdid = this.args.udid;
    return cb();
  }
...
IOS.prototype.prelaunchSimulator = function (cb) {
  var msg;
  if (this.shouldUseSpecificSimulator()) {
    logger.debug("We use specific simulator UDID. This assumes that simulator is already launched.");
    return cb();
  }
...
IOS.prototype.runSimReset = function (cb) {
  if (!this.shouldUseSpecificSimulator() && (this.args.reset || this.args.fullReset) ) {
    ...
    async.series([
      ...
      function (cb) {
        if (this.args.fullReset && !this.shouldUseSpecificSimulator()) {
          this.deleteSim(cb);
        } else {
          cb();
        }
...
IOS.prototype.isolateSimDevice = function (cb) {
  if (!this.shouldUseSpecificSimulator() && this.args.isolateSimDevice &&
...
IOS.prototype.navToInitialWebview = function (cb) {
  ...
  setTimeout(function () {
    ...
    } else if (parseInt(this.iOSSDKVersion, 10) >= 7 && !this.onRealDevice() && this.capabilities.safari) {
    ...
    this.getLatestWebviewContextForTitle(titleRegex, function (err, res) {
      if (err) {
        ...
        if ((Date.now() - start) < 90000) {
          ...
          if (this.onRealDevice() || !this.capabilities.safari) {
